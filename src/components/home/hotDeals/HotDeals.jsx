...
const hotDealsDataItems = hotDeals.map(deal=>{
    return <ProductCard key = {deal.id} productData = {deal}/>
})

  return (
    <HotDealsWrapper>
      {/* hotdeals section title */}
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{width:"100%" , padding:"1rem"}}>
          <Typography variant="h2" sx={{color:"#fff", fontSize:40, fontWeight:700}}>Hot deals</Typography>
          <Typography variant="h6" sx={{color:"#fff", fontSize:20, fontWeight:700}}>More..</Typography>
      </Stack>
